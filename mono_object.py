...
def load_random_object(object_dir,radius):
    #print(object_dir)
    ...
    if not object_files:
        ...
        # print(f"Loaded {len(obj)} objects from {chosen_file}:")
        # for object in obj:
        #     print(f" - {object.get_name()}")

    #need to set location such that the object is not merged with ground and randomly within the radius of camera
    # obj.set_location([0,0,0])  #objects location fixed at center
    # random_x = np.random.uniform(-radius+3, radius-3)
    # random_y = np.random.uniform(-radius+3, radius-3)
    ...
    # scale_factor = 1.5  # CLEVR size range
    # obj.set_scale([scale_factor] * 3)  # Uniform scaling
    return obj,size
...
def apply_random_material_self(obj):
    ...
    if material_type == "metallic":
        ...
        mat.set_principled_shader_value("Roughness", 0.2)  # Slight shine
...
def render_scene(output_image_path, output_annotation_dir, material_dir, object_dir , light_type, light_num):
    ...
    # print("all objects at start")
    # print(bproc.object.get_all_mesh_objects())
    # Set up camera
    #camera = bproc.camera.create_camera()
    ...
    # print(f"Number of mesh objects in the scene: {len(mesh_objects)}")
    # for object in mesh_objects:
    #     print(f"Mesh Object Name: {object.get_name()}")


    # lighting settings-problem: if point light is set all lights become point lights
    ...
    obj,size=load_random_object(object_dir,min_radius)
    ...
    # print(camera_poses)
    #tmat = camera_poses[0]  # Get the first camera pose here we have only 1 camera pose
    tmat = camera_poses #remove if more than one
    #print(tmat)
